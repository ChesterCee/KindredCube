    ...
    return this.database.withUser(request.user.id, async (client) => {
      ...
      const result = await client.query<DiscoveryRow>(
        `SELECT d.user_id,
                ...
                ARRAY(SELECT cm.constellation_id::text FROM constellation_members cm WHERE cm.user_id = d.user_id AND (cm.status = 'accepted' OR (cm.status = 'suggested' AND (cm.user_id = $1 OR EXISTS (SELECT 1 FROM constellations c WHERE c.id = cm.constellation_id AND c.creator_id = $1))))) AS constellation_ids,
