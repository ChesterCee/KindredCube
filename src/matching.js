...
const normalized = (value) => Math.max(0, Math.min(1, value));
const normalizedWords = (items) => new Set((items || []).map((item) => item.trim().toLowerCase()).filter(Boolean));
...
function vectorSimilarity(first, second) {
    if (!first?.length || !second?.length || first.length !== second.length)
        return null;
    const averageDifference = first.reduce((sum, value, index) => sum + Math.abs(value - second[index]), 0) / first.length;
    return normalized(1 - averageDifference);
}
function lifestyleSimilarity(first, second) {
    const keys = new Set([
        ...Object.keys(first.lifestyle || {}),
        ...Object.keys(second.lifestyle || {}),
    ]);
    const comparable = [...keys].filter((key) => first.lifestyle?.[key] && second.lifestyle?.[key]);
    const exact = comparable.filter((key) => first.lifestyle?.[key] === second.lifestyle?.[key]).length;
    ...
    if (interestScore === null && lifestyleScore === null)
        return null;
...
function locationSimilarity(first, second) {
    ...
    if (distance === undefined || maximum === undefined)
        return null;
    if (distance <= maximum)
        return normalized(1 - (distance / Math.max(1, maximum)) * 0.55);
    return first.openToRelocate || second.openToRelocate ? 0.55 : 0;
}
function scoreMatch(viewer, candidate) {
    const exclusions = [];
    if (viewer.blockedIds?.includes(candidate.id) || candidate.blockedIds?.includes(viewer.id))
        exclusions.push("blocked");
    if (candidate.suspended)
        exclusions.push("safety_restricted");
    if (candidate.visible === false)
        exclusions.push("not_visible");
    if (candidate.contactVerified === false)
        exclusions.push("contact_not_verified");
    if (viewer.seeking?.length && candidate.gender && !viewer.seeking.includes(candidate.gender))
        exclusions.push("gender_preference");
    if (candidate.seeking?.length && viewer.gender && !candidate.seeking.includes(viewer.gender))
        exclusions.push("reciprocal_gender_preference");
    if (candidate.age !== undefined && viewer.minAge !== undefined && candidate.age < viewer.minAge)
        exclusions.push("below_age_range");
    if (candidate.age !== undefined && viewer.maxAge !== undefined && candidate.age > viewer.maxAge)
        exclusions.push("above_age_range");
    ...
    if (viewer.acceptsPartnerWithChildren === false && candidate.hasChildren)
        exclusions.push("children_preference");
    if (viewer.allowsSmoking === false &&
        candidate.lifestyle?.Smoke &&
        !["never", "no"].includes(candidate.lifestyle.Smoke.toLowerCase()))
        exclusions.push("smoking_preference");
    if (viewer.essentialReligions?.length &&
        candidate.religion &&
    ...
    if (viewer.requiredRelationshipGoals?.length &&
    ...
    const compatibilityScore = culturalValues * finalWeights.culturalValues +
    ...
    const readiness = (candidate.idVerified ? 3 : 0) +
...
function rankMatches(viewer, candidates, toSignals) {
    return candidates
